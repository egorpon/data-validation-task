...
class Address(Validationable):
    ...
    def is_valid(self) -> bool:
        return all(
            [self.is_city_valid(), self.is_country_valid(), self.is_postal_code_valid()]
        )
...
class User(Validationable):
    ...
    def is_valid(self) -> bool:
        return all(
            [
                self.is_email_valid(),
                self.is_full_name_valid(),
                self.is_gender_valid(),
                self.is_date_of_birth_valid(),
                self.is_addresses_valid(),
            ]
        )
    ...
    def is_full_name_valid(self) -> bool:
        return bool(re.fullmatch(r"^[a-zA-z]+\s[a-zA-z]+$", self.full_name))
    ...
    def is_date_of_birth_valid(self) -> bool:
        return (
            ...
            and self.date_of_birth < datetime.now().date()
        )
